...
def UVAttack(images, labels, net, H, W, max_iteration, learning_rate, arrow_size, attack_YUV, target=None, num_random_init=1, init_method='normal', loss_type="ce", smoothness=0, rand_param=1):
...
def overall_testset_accuracy(wb, log_file, dataset_type="CIFAR10", iteration=100, learning_rate=0.1, arrow_size=10, target=None, attack_YUV="UV only", color_threshold=0, max_init=1, init_method='normal', network="resnet50", loss_type="ce", smoothness=0, rand_param=1):
    ...
    for data in tqdm(dataiter):
        ...
        for i in range(images.shape[0]):
            if(img_clrfl[i]*255 > color_threshold):
                ...
                adv_img, it_count, init_count = UVAttack(images, labels, net, H, W, iteration, learning_rate, arrow_size, attack_YUV, target=target, num_random_init=max_init, init_method=init_method, loss_type=loss_type, smoothness=smoothness, rand_param=rand_param)
